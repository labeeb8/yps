...
def sync_playlist(config_path):
    ...
    if 'format' in config_data:
        run_in_subprocess([YOUTUBE_DL_PATH, '-f', config_data['format'], config_data['youtube_playlist'], '--no-check-certificat'])
    else:
        run_in_subprocess([YOUTUBE_DL_PATH, config_data['youtube_playlist'], '--no-check-certificat'])
